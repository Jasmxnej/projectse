<template>
<nav
  class="fixed top-0 left-0 w-full z-50 px-8 py-3 flex justify-between items-center transition-all duration-300 bg-white text-accent2 shadow-sm"
>
    ...
    <div class="flex items-center space-x-12">
      <RouterLink
        ...
        :class="{ 
          'text-teal border-b-2 border-teal pb-1': isActive('/about'),
          'hover:text-teal': !isActive('/about')
        }"
      >
        ABOUT US
      </RouterLink>
      <RouterLink
        ...
        class="nav-link text-sm font-medium tracking-wide uppercase transition-colors duration-200"
        :class="{ 
          'text-teal border-b-2 border-teal pb-1': isActive('/contact'),
          'hover:text-teal': !isActive('/contact')
        }"
      >
        CONTACT US
      </RouterLink>
    ...
    <div class="flex items-center space-x-8 relative">
      <template v-if="!authStore.currentUser">
        <RouterLink 
          to="/login" 
          class="nav-link text-sm font-medium tracking-wide uppercase transition-colors duration-200 hover:text-teal"
        >
          LOG IN
        </RouterLink>
        <RouterLink 
      ...
      <template v-else>
        <div 
          ...
          class="nav-link cursor-pointer select-none flex items-center space-x-3 hover:text-teal transition-colors duration-200"
        >
          <span class="text-sm font-medium tracking-wide uppercase">
            {{ user?.username }}
          </span>
          <div class="w-8 h-8 flex items-center justify-center bg-teal text-white rounded-full text-xs font-semibold uppercase">
        ...
        >
          <RouterLink
            to="/profile"
            class="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-blue-50 hover:text-teal border-b border-gray-100 transition-colors duration-200"
